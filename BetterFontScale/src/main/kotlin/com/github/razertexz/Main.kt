...
class Main : Plugin() {
    ...
    private fun patches() {
            ...
            WidgetChatListAdapter::class.java.superclass?.superclass?.getDeclaredMethod("onBindViewHolder", RecyclerView.ViewHolder::class.java, Int::class.java)?.let { method ->
                patcher.patch(method, Hook {
                    ...
                    if (itemView is ViewGroup) {
                        ...
                        val textView = rootView.findViewById<TextView?>(0x7f0a0357)
        ...
        if (chatBoxFontScale != 0.0f) {
            ...
            patcher.after<WidgetChatInput>("configureUI", ChatInputViewModel.ViewState::class.java) { param ->
                val thisObject = param.thisObject as WidgetChatInput
        ...
        patcher.after<ChannelMembersListAdapter>("onBindViewHolder", RecyclerView.ViewHolder::class.java, Int::class.java) { param ->
            val viewHolder = param.args[0] as RecyclerView.ViewHolder

            val itemView = viewHolder.itemView
            if (itemView is ViewGroup) {
                val rootView = itemView

                val userName = rootView.findViewById<TextView?>(0x7f0a10cc)
        ...
        if (aboutMeFontScale != 0.0f) {
            patcher.after<WidgetUserSheet>("configureUI", WidgetUserSheetViewModel.ViewState::class.java) { param ->
                val thisObject = param.thisObject as WidgetUserSheet
        ...
        if (profileStatusFontScale != 0.0f) {
            patcher.after<UserProfileHeaderView>("updateViewState", UserProfileHeaderViewModel.ViewState.Loaded::class.java) { param ->
                val thisObject = param.thisObject as UserProfileHeaderView
